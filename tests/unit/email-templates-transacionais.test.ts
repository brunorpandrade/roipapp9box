...
// Confirmação alteração) + §12.5 (Template 4 — Aviso segurança) +
...
describe('Template 1 — Reset de senha (§12.2)', () => {
  ...
  it('assunto canonico literal', () => {
    ...
    expect(r.assunto).toBe('[ROIP APP] Redefinição de senha');
    expect(TEMPLATE_1_ASSUNTO).toBe('[ROIP APP] Redefinição de senha');
  });
  ...
  it('corpo texto contem nome + link canonico', () => {
    ...
    expect(r.corpoTexto).toContain('link é válido por 24 horas');
...
describe('Template 3 — Confirmação alteração (§12.4)', () => {
  beforeEach(() => _resetHandlebarsCache());
  ...
  it('assunto canonico literal', () => {
    ...
    expect(r.assunto).toBe('[ROIP APP] Confirme a alteração do seu e-mail de acesso');
    expect(TEMPLATE_3_ASSUNTO).toBe('[ROIP APP] Confirme a alteração do seu e-mail de acesso');
  });
  ...
  it('corpo texto contem link canonico /confirmar-alteracao-email', () => {
    ...
    expect(r.corpoTexto).toContain(
      '[Confirmar alteração]  → link: https://app.roip.com.br/confirmar-alteracao-email?token=abc.def',
    );
  });
  ...
  it('corpo HTML sem target=_blank (B3)', () => {
    ...
    expect(r.corpoHtml).not.toContain('target="_blank"');
    expect(r.corpoHtml).toContain('Confirmar alteração</a>');
  });
});

describe('Template 4 — Aviso segurança (§12.5)', () => {
  beforeEach(() => _resetHandlebarsCache());
  ...
  it('corpo texto contem dataHora + novoEmail', () => {
    ...
    expect(r.corpoTexto).toContain('foi alterado em 31/07/2026 às 14:30');
    expect(r.corpoTexto).toContain('Novo e-mail: novo@example.com');
    expect(r.corpoTexto).toContain('aviso de segurança');
  });
});

describe('formatDataHoraCanonica — §12.5', () => {
  it('formata Date UTC no fuso SP como DD/MM/YYYY às HH:mm', () => {
    // 2026-07-31 17:30:00 UTC = 2026-07-31 14:30 BRT (UTC-3)
    const d = new Date('2026-07-31T17:30:00Z');
    expect(formatDataHoraCanonica(d, 'America/Sao_Paulo')).toBe('31/07/2026 às 14:30');
  });

  it('zero-padding em dia/mes/hora/minuto', () => {
    // 2026-01-05 12:05:00 UTC = 2026-01-05 09:05 BRT
    const d = new Date('2026-01-05T12:05:00Z');
    expect(formatDataHoraCanonica(d, 'America/Sao_Paulo')).toBe('05/01/2026 às 09:05');
  });
});
